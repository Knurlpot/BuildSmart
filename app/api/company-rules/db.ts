...
export async function fetchCompanyRules(companyId: number): Promise<CompanyRulesPayload> {
  ...
  const materialRules: MaterialRuleEntry[] = materialResult.rows.map((row) => {
    ...
    return {
      ...
      selected_supplier_id: typeof parsed.selected_supplier_id === "number" ? parsed.selected_supplier_id : null,
      selected_supplier_name: typeof parsed.selected_supplier_name === "string" ? parsed.selected_supplier_name : null,
  ...
  const laborRules: LaborRule[] = laborResult.rows.map((row) => {
    ...
    return {
      ...
      worker_count: typeof parsed.worker_count === "number" ? parsed.worker_count : 1,
  ...
  return {
    ...
    existingRules: [
      ...
      ...laborRules.map((rule) => ({
        ...
        detail: `${rule.worker_count} worker${rule.worker_count === 1 ? "" : "s"} · ₱${rule.labor_rate}`,
        effective_date: rule.effective_date,
        status: rule.is_active ? "Active" as const : "Disabled" as const,
      })),
      ...
      const rule = await client.query<{ rule_id: number }>(
        ...
        [
          ...
          JSON.stringify({
            ...
            selected_supplier_id: Number.isFinite(Number(body.selected_supplier_id)) ? Number(body.selected_supplier_id) : null,
            selected_supplier_name: typeof body.selected_supplier_name === "string" ? body.selected_supplier_name : null,
            warranty_years: body.warranty_years ?? null,
            lifespan_years: body.lifespan_years ?? null,
          }),

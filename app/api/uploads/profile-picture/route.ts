...
export async function POST(request: NextRequest) {
  try {
    if (!readSession(request)) return NextResponse.json({ error: "Please sign in to upload an image." }, { status: 401 });
    return NextResponse.json({ url: await storeImage(request, "profile-pictures") }, { status: 201 });
  } catch (error) {
